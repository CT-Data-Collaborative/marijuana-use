...
##################################################################
#
# Processing Script for Marijuana Use
...
for (i in 1:length(legacy_table_dfs)) {
  ...
  descriptions <- c("First Use of Marijuana", 
...
new_table_dfs <-c("2012-2014 Substate SAE Table 2",
                  "2012-2014 Substate SAE Table 3",
                  "2012-2014 Substate SAE Table 5",
                  "2014-2016 Substate SAE Table 1",
                  "2014-2016 Substate SAE Table 2",
                  "2014-2016 Substate SAE Table 3")
...
test$`Marijuana Use` <- NA
test$`Marijuana Use`[grep("First", test$`Table Description`)] <- "First Use of Marijuana"
test$`Marijuana Use`[grep("Year", test$`Table Description`)] <- "Marijuana Use in the Past Year"
test$`Marijuana Use`[grep("Month", test$`Table Description`)] <- "Marijuana Use in the Past Month"
...
#assign variable
all_data_long$Variable[grep("Estimate", all_data_long$Variable)] <- "Marijuana Use"
...
all_data_final <- all_data_long %>% 
  select(`Substate Region`, Year, `Age Range`, `Marijuana Use`, `Measure Type`, Variable, Value) %>% 
  rename(Region = `Substate Region`) %>% 
  arrange(Region, Year, `Age Range`, `Marijuana Use`, Variable)
...
write.table(
  all_data_final,
  file.path(getwd(), "data", "marijuana-use-2016.csv"),
  sep = ",",
  na = "-9999",
  row.names = F
)
